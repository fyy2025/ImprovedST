...
path.cc <- "/nas/longleaf/home/fyy2025/survival_tree_revision/tree_500_30/"
...
for (dataset in 1:num_dataset){
  try(
    {
    train = read_csv(paste0(path.cc,"data/train",(num_dataset*(iter-1)+dataset),".csv"))
    test = read_csv(paste0(path.cc,"data/test",(num_dataset*(iter-1)+dataset),".csv"))
    structure=TRUE
    ...
    true.lin.pred <- as.data.frame(10*(0.3*(X[,1]>0) -0.2*(X[,2]<0)+0.3*(X[,4]<0)*(X[,6]<0)-0.4*(X[,7]>0)*(X[,3]>0)))
...
write.csv(summary_no_zero,file=paste0(path.cc, 'results/summary',iter,'.csv'), row.names = F)
